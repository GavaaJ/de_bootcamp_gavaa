...
#     AIRFLOW__DATABASE__SQL_ALCHEMY_CONN=postgresql+psycopg2://airflow:airflow@54.206.125.214:5432/airflow_db
#     ENV
...
module "code_bucket" {
  ...
  project     = var.project
  environment = var.environment
  bucket_name = "code-${var.environment}-gavaa"
}
...
module "data_bucket" {
  ...
  project     = var.project
  environment = var.environment
  bucket_name = "${var.project}-${var.environment}-gavaa"
}
